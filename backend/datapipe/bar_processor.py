...
Steps:
  ...
  6. Advance ``baseline_history_sum`` so the next bar's RVOL denominator
     grows correctly.
  7. Emit the enriched bar via an optional callback -- this is the hook the
...
async def process_bar(
    ...
    # (SSE listeners immediately hit the DB for context around the bar).
