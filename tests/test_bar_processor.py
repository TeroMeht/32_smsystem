...
together and that consecutive bars grow the baseline_history_sum
correctly (which is what makes RVOL cumulative across the session).
"""
...
@pytest.mark.asyncio
async def test_process_bar_enriches_and_grows_baseline():
    pool = _StubPool()
    store = SessionStore()

    sink = AsyncMock()

    # Seed baseline so RVOL has a denominator
    ...
    st.rvol_baseline = {time(9, 30): 500.0, time(9, 31): 500.0}
    st.atr = 1.0
    ...
    # Second bar should see history=[b0] and larger baseline_history_sum
    ...
    # baseline_history_sum grew by both slots' baselines (500 + 500)
    assert st_after.baseline_history_sum == 1000.0
    assert out2.rvol_cum is not None
